...
class Program{
    ...
    function create($title,$descrip, $start_date,$end_date,$type, $region){
        ...
        $stm->execute(['title'=>$title,'descrip'=>$descrip, 
                        'startt_date'=>$start_date,'end_date'=>$end_date,
                        'typ'=>$type, 'region'=>$region
                    ]);
    }

    function update(){
        $stm= App::db()->prepare("UPDATE programs SET title='new title', descrip='this is new description'");
        $stm->execute();
    }
